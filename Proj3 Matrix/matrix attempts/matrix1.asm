.data
	colArray:	.space 316 #might need 320 if this is blue
...
.text
	add $s7, $zero, $zero #counter which is also my fucking asshole column
	la $s6, colArray	#contains column array
	mul $t7, $s7, 4		#multiplied the column location by 4 
	
	add $t7, $s6, $t7	# 
	addi $s7, $s7, 1
	lbu $t6, 0($t7)
	beq $t6, 1, hop		#if the column is running/falling hop
	#generate a random number say 1-80...to determine if we activate this column
	#if activate then leave...if not no idea
	#random number in v0
	jal _random #make sure you store the register address
	
	#call the active column...
	#outer loop resets and inner loop moves across the columns
...
hop:	
	...
	li $t1, 1
	sb $t1, 0($t0)
	jal _random			#call random number method
	add $t2, $v0, $zero		#returns v0 from random and places it
        sb $t2, 1($t0)
        li $t2, -13			# starts off 13 lines up above terminal
...
jump:	add $t3, $zero, $s3	#copying s3 into t3 (temp variable)
	...
	reset:
		...
		addi $t2, $t2, 1	#counter ++
		addi $s3, $s3, 320	# move to next row in same column
